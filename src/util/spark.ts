...
export function sparkPath(seed: string, w: number, h: number): string {
  let s = 0;
  ...
  const pts = 24;
  const xs = (i: number): number => (i / (pts - 1)) * w;
  const ys = (v: number): number => h - 4 - v * (h - 8);
  let d = '';
  for (let i = 0; i < pts; i++) {
...
export function sparkSvg(seed: string, color: string): string {
  const w = 200,
    h = 32;
