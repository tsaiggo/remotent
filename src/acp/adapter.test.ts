...
describe('acpToSession', () => {
  ...
  test('regression: a replayed user_message_chunk renders as a human turn and sanitizes OMO artifacts', () => {
    const rawText = `我发现显示出我发送了[Pasted ~1<system-reminder> [BACKGROUND TASK COMPLETED] **ID:** bg_48ae2209 </system-reminder> <!-- OMO_INTERNAL_INITIATOR -->Continue if you have next steps, or stop and ask for clarification if you are unsure how to proceed.`;
    const s = acpToSession({
      ...baseSnap,
      messages: [
        {
    ...
    expect(s.turns[0]?.text).toBe('我发现显示出我发送了');
  });
});
